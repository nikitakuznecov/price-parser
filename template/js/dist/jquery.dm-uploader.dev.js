...
(function (factory) {
  ...
  var defaults = {
    ...
    //params: id, data
    onUploadCanceled: function onUploadCanceled() {},
    //params: id
    onUploadError: function onUploadError() {},
    //params: id, xhr, status, message
    ...
    onDocumentDragLeave: function onDocumentDragLeave() {}
  };

  var DmUploaderFile = function DmUploaderFile(file, widget) {
    this.data = file;
    this.widget = widget;
    this.jqXHR = null;
    this.status = FileStatus.PENDING; // The file id doesnt have to bo that special.... or not?
  ...
  DmUploaderFile.prototype.upload = function () {
    ...
    var fd = new FormData();
    fd.append(file.widget.settings.fieldName, file.data); // Append extra Form Data
    ...
    file.widget.settings.onBeforeUpload.call(file.widget.element, file.id); // Ajax Submit
  ...
  var DmUploader = function DmUploader(element, options) {
  ...
  DmUploader.prototype.init = function () {
    ...
    var input = widget.element.is("input[type=file]") ? widget.element : widget.element.find("input[type=file]"); //-- Is the input our main element itself??

    if (input.length > 0) {
      input.prop("multiple", this.settings.multiple); // Or does it has the input as a child
  ...
  DmUploader.prototype.initDnD = function () {
    var widget = this; // -- Now our own Drop
  ...
  DmUploader.prototype.processQueue = function () {
    ...
    this.queueRunning = true; // Start next file
  ...
  DmUploader.prototype.cancelAll = function () {
    ...
    this.queueRunning = false; // cancel 'em all
